...
to run the beta link analysis - you can either convert the networks from correlation tables: ^
or read them from an excel file: v

```{r read_networks_file}
library("readxl")
...
```{r relative_degree_for_protein}

genes_path <- "C:/Users/Geut/Documents/R_Github_Root/cancer_neworks/HPC/"
...
degree_table_prot <- prots_meta[, 3:4]
...
for (i in 1:length(networks)) {
  ...
  new_col_names <- c(colnames(degree_table_prot), netname)
  
  degree_table_prot <- merge(degree_table_prot,curr_cancer_degrees,
                        by.x="ENSID", by.y=0,all.x=TRUE)
  colnames(degree_table_prot) <- new_col_names
}
View(degree_table_prot)

# save the table to a file
write.csv(degree_table_prot, "output/relative_degree_proteins.csv", row.names = TRUE)

```

```{r relative_degree_for_chaperon}
# only it is not relative, but absolute degree to keep it discreet
...
for (i in 1:length(networks)) {
  ...
  for (chap in 1:nrow(chaps_meta)) {
    ...
    chap_degrees[SYMB] <- sum(net[SYMB, ])#/nrow(prots_meta)
  }
...
# save the table to a file
write.csv(degree_table_chap, "output/relative_degree_chaperons.csv", row.names = FALSE)
...
write.csv(sum_table, "output/corr_count.csv", row.names = TRUE)
```
...
all_degree_table <- merge(degree_table_prot[, -2], union_degrees,
                      by.x="ENSID", by.y=0,all.x=TRUE)
...
chapXcancer <- as.matrix(degree_table_chap[, -1])
pheatmap(chapXcancer, cutree_cols = 2,  cutree_rows = 2,
         show_rownames = TRUE, show_colnames = TRUE,
         clustering_method = "ward.D", 
         clustering_distance_rows = "manhattan", main = "chaperon degree by cancer")
...
# turn the relative degree to binari to calculate participation coefficiant
...
# sum the rows - in how many many cancers a protein perticipates
...
as.data.frame(table(participation_coefficiant))
...
list of proteins that have zero interactions
...
TODOS:
- get distribution values using table() - V
- calc relative degree for chaperon - V
- calc d' specificity for chaperon in union - read paper to make sure this is the best analysis
- d' for chap per cancer, built chapX cancer table
- put the table above in a stochastic block modeling algorithm
- find different color for 0's in the heatmaps, maybe using ggplot 
- add annotation for cyto-prots - "howw?" ID RATHER NOTT
- sort heatmap cols and rows by colSum and rowSums
