...
class MultiTensor :
	def __init__(self,N=100,L=1,K=2, N_real=1,tolerance=0.1,decision=10,maxit=500,rseed=0,out_adjacency=False,inf=1e10,err_max=0.00001,err=0.1,initialization=0,undirected=False,folder="data/",end_file="",adj="adjacency.dat",w_file="w.dat",assortative=False,name="adjacency"):	
		self.N=N
		self.L=L
		self.K=K
		self.N_real=N_real
	...
	def _initialize(self,u_list,v_list,nodes):         
		...
		infile1=self.folder + self.name + "_" +'u_K'+str(self.K)+self.w_file
		infile2=self.folder + self.name + "_" +'v_K'+str(self.K)+self.w_file
		w_infile=self.folder + self.name +"_" +'w_K'+str(self.K)+self.w_file
	...
	def output_results(self,maxL,nodes):
		...
		infile1=self.folder + self.name + "_" +"u_K"+str(self.K)+self.end_file
		infile3=self.folder + self.name + "_" +"w_K"+str(self.K)+self.end_file
		in1=open(infile1,'w')				
		in3=open(infile3,'w')
		print >>in1,"# Max Likelihood= ",maxL," N_real=",self.N_real				
		print >>in3,"# Max Likelihood= ",maxL," N_real=",self.N_real				
		if(self.undirected==False):
			infile2=self.folder + self.name+ "_" +"v_K"+str(self.K)+self.end_file
			in2=open(infile2,'w')				
